...
class SpiderInfo(models.Model):
    ...
    yzmmodel = models.ForeignKey(YzmModel,verbose_name='绑定模型',blank=True)
    update_model = models.FileField(upload_to='models/update/%Y/%m',verbose_name='支持模型文件上传',default='',blank=True)
    update_net = models.FileField(upload_to='models/update/%Y/%m', verbose_name='支持网络文件上传', default='',blank=True)
...
class SpiderStatus(models.Model):
    ...
    desc = models.TextField(verbose_name='备注',blank=True)
    ratio_nums = models.IntegerField(verbose_name='循环多少次',default=0)
    add_time = models.DateTimeField(default=datetime.now, verbose_name='添加时间')
